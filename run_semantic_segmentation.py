...
def main():
    ...
    if "annotated" in dataset["train"].column_names:
        dataset = dataset.rename_columns({"annotated": "label"})
    if "val" in dataset.keys():
        tmp = dataset.pop("val")
        dataset["validation"] = tmp
    if "Scene_category" in dataset.keys():
        dataset.pop("Scene_category")
    ...
    #metric.cache_file_name = "/aistudio/workspace/research/dinov2_csaroff/cache"
    print("cache_file_name={}".format(metric.cache_file_name))
